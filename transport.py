...
print("部屋の名前、URL（あれば）、家賃、居住坪数を入力してください。")
room_name = input("部屋の名前:")
url = input("URL:")
rent = float(input("家賃(円):"))   
area = float(input("居住面積(m^2):"))
area_tubo = float(Decimal(area / 3.3).quantize(Decimal("0.01"), rounding=ROUND_HALF_UP))
...
#subsidy = (Decimal(subsidy) / Decimal("100")).quantize(Decimal("1"), rounding=ROUND_HALF_UP) * Decimal("100")
